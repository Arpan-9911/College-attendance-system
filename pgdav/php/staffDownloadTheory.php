...
if($time == 'Sem'){
    ...
    $query = "SELECT DISTINCT DATE(date) as total_taken FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-1'";
    $query2 = "SELECT DISTINCT DATE(date) as total_taken FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-2'";
    ...
    $query = "SELECT DISTINCT DATE(date) as total_taken FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-1' AND month(date) = $month";
    $query2 = "SELECT DISTINCT DATE(date) as total_taken FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-2' AND month(date) = $month";
    $result = mysqli_query($conn, $query);
    $result2 = mysqli_query($conn, $query2);
    $count1 = mysqli_num_rows($result);
    $count2 = mysqli_num_rows($result2);
...
if($time == 'Sem'){
    $dateQuery = "SELECT DISTINCT DATE(date) AS attendanceDate FROM `attendance` WHERE teacherName = '$staffName' AND subject LIKE '%$subject%' AND period = 'Theory-1' ORDER BY attendanceDate ASC";
}
else{
