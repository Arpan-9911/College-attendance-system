...
$dateQuery = "SELECT DISTINCT DATE(date) AS attendanceDate FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-1' AND month(date) = $month ORDER BY attendanceDate ASC";
...
if (mysqli_num_rows($dateResult) > 0) {
    ...
    if ($studentResult) {
        ...
        while ($studentRow = mysqli_fetch_assoc($studentResult)) {
            ...
            while ($dateRow = mysqli_fetch_assoc($dateResult)) {
                ...
                $selectDate2 = "SELECT DISTINCT DATE(date) AS attendanceDate2 FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-2' AND date = '$attendanceDate'";
                ...
                if (mysqli_num_rows($resultDate2) > 0) {
                    ...
                    $attendanceQuery = "SELECT attendance FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-1' AND date = '$attendanceDate' AND studentName = '$name' AND studentRoll = '$roll'";
                    ...
                    $attendanceDate = $dateRow['attendanceDate'];
                    $sheet->setCellValue($colIndex . 1, $attendanceDate);
                    $attendanceQuery2 = "SELECT attendance FROM `attendance` WHERE teacherName = '$staffName' AND subject = '$subject' AND period = 'Theory-2' AND date = '$attendanceDate' AND studentName = '$name' AND studentRoll = '$roll'";
    ...
    echo "<script>window.location.href ='staffDownloadTheory.php?subject={$subject}';</script>";
    exit(); // Always exit after header redirects
}
